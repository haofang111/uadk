...
static int init_two_ctx_config(int ctx_mode, int type, struct wd_sched *sched)
{
	...
	memset(sched, 0, sizeof(struct wd_sched));
	ctx_conf.ctx_num = 2;
			     ...
			     int mode, int hw_flag, int iteration)
{
	...
	ret = init_two_ctx_config(mode, op_type, &sched);
	if (ret)
		return ret;
